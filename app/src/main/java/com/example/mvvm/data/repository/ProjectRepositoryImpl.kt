...
constructor(private val remote: ProjectDataSource) : ProjectRepository, BaseRepository() {
    ...
    override suspend fun addResearcherToProject(projectId: Long, researcherEmail: String): Flow<List<Researcher>> = runFlow {
        remote.addResearcherToProject(projectId, researcherEmail).map { it.map { it.getResearcher() } }
    }
    ...
    override suspend fun setCouncil(projectId: Long, council: CouncilRequest) = runFlow {
        remote.setCouncil(projectId, council)
    }

    override suspend fun setProjectTime(projectId: Long, date: String) = runFlow {
        remote.setProjectTime(projectId, date)
    }

    override suspend fun setProjectScore(projectId: Long, score: Double) = runFlow {
        remote.setProjectScore(projectId, score)
    }
}
